...
const ALL_ITEMS: Item[] = [
  ...
  { id: 4, name: "카우보이 모자", description: "자유로운 영혼을 위한 모자. 서부의 황야가 떠오르는 디자인입니다.", icon: <CowboyHatIcon /> },
  { id: 5, name: "요리사 모자", description: "맛있는 요리를 만들 때 필수! 깨끗하고 하얀 색상이 매력적입니다.", icon: <ChefHatIcon /> },
  { id: 6, name: "산타 모자", description: "크리스마스의 설렘을 담은 모자. 방울이 달려 있어 귀여움을 더합니다.", icon: <SantaHatIcon /> },
  { id: 7, name: "노란 베레모", description: "예술적 감각이 느껴지는 세련된 베레모입니다. 부드러운 노란색이 포인트예요.", icon: <YellowBeretIcon /> },
  { id: 8, name: "레프러콘 모자", description: "행운을 가져다줄 것 같은 초록색 모자입니다. 황금 버클이 매력적이죠.", icon: <LeprechaunHatIcon /> },
  { id: 9, name: "검은 카우보이 모자", description: "시크하고 강인한 인상을 주는 검은색 카우보이 모자입니다.", icon: <BlackCowboyHatIcon /> },
  { id: 10, name: "빨간 버킷햇", description: "어떤 스타일에도 힙하게 어울리는 빨간색 버킷햇입니다.", icon: <RedBucketHatIcon /> },
  { id: 11, name: "조선시대 갓", description: "선비의 기품이 느껴지는 전통 갓입니다. 검은색 실루엣과 갓끈이 멋스러워요.", icon: <KoreanGatIcon /> },
  { id: 12, name: "야구 헬멧", description: "단단하고 묵직한 회색 야구 헬멧입니다. 어떤 충격도 막아줄 것 같아요.", icon: <BaseballHelmetIcon /> },
  { id: 13, name: "바이킹 투구", description: "용맹한 바이킹의 상징인 뿔 달린 투구입니다. 파란색 본체가 아주 강렬하죠.", icon: <VikingHatIcon /> },
  { id: 14, name: "솜브레로", description: "멕시코의 정취가 느껴지는 넓은 챙의 모자입니다. 화려한 색상이 특징이에요.", icon: <SombreroIcon /> },
  { id: 15, name: "페즈", description: "터키의 전통적인 붉은색 원통형 모자입니다. 검은색 술이 달려있어요.", icon: <FezIcon /> },
  { id: 16, name: "논라", description: "베트남의 전통적인 원뿔형 모자입니다. 햇빛을 피하기에 아주 좋죠.", icon: <NonLaIcon /> },
  { id: 17, name: "신사 모자", description: "영국의 신사가 떠오르는 높은 검은색 모자입니다. 클래식한 멋이 있죠.", icon: <TopHatIcon /> },
  { id: 18, name: "우샨카", description: "추운 겨울을 나기 위한 러시아의 따뜻한 털모자입니다.", icon: <UshankaIcon /> },
  { id: 19, name: "경찰모", description: "정의를 수호하는 경찰의 모자입니다. 반짝이는 금색 배지가 포인트예요.", icon: <PoliceCapIcon /> },
  { id: 20, name: "프로펠러 모자", description: "머리 위에 프로펠러가 달린 장난기 가득한 알록달록한 모자입니다.", icon: <PropellerHatIcon /> },
  { id: 21, name: "학위모", description: "졸업식의 영광이 담긴 사각 모자입니다. 노란색 술이 달려있어요.", icon: <GraduationCapIcon /> },
  { id: 22, name: "터번", description: "인도나 중동 지역에서 머리에 두르는 전통적인 형태의 모자입니다.", icon: <TurbanIcon /> },
  { id: 23, name: "일장기 머리띠", description: "강한 의지가 느껴지는 일본 스타일의 하얀색 머리띠입니다.", icon: <HachimakiIcon /> },
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="relative w-full max-w-[320px] bg-white border-[6px] border-black p-8 animate-in zoom-in duration-200 shadow-[16px_16px_0px_0px_rgba(0,0,0,0.3)]">
            <button onClick={() => setSelectedItem(null)} className="absolute top-2 right-2 w-8 h-8 font-black text-2xl hover:text-red-500">×</button>
            <div className="flex flex-col items-center text-center">
              <div className="w-24 h-24 bg-slate-100 border-4 border-black flex items-center justify-center p-4 mb-6">{selectedItem.icon}</div>
              <h3 className="text-lg font-black mb-3 border-b-4 border-penguin-yellow">{selectedItem.name}</h3>
              <p className="text-xs font-bold text-gray-600 mb-6">{selectedItem.description}</p>
              <button onClick={() => handleEquip(selectedItem)} className="w-full py-3 bg-black text-white font-black hover:bg-penguin-yellow hover:text-black">
                EQUIP
              </button>
            </div>
          </div>
