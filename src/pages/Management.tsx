...
const Management: React.FC = () => {
  ...
  const handleDelete = async () => {
    if (
      window.confirm(
        "정말로 이 투표를 삭제하시겠습니까? 관련 데이터가 모두 삭제됩니다.",
      )
    ) {
      setLoading(true);
      try {
  ...
  if (!poll)
    return (
      <div className="px-2 py-8 text-center font-bold text-penguin-black">
        투표를 찾을 수 없습니다.
      </div>
    );
    ...
    return (
      <div className="py-4 max-w-md mx-auto">
        ...
        <form
          ...
          className="bg-white p-6 md:p-8 rounded-[3rem] shadow-sm border border-gray-100 space-y-5"
        >
          <div>
          ...
          <button
            type="submit"
            className="w-full py-5 bg-penguin-black text-penguin-yellow rounded-[1.5rem] font-black text-xl shadow-xl shadow-gray-200 hover:bg-penguin-yellow hover:text-penguin-black active:scale-[0.98] transition-all mt-4"
          >
            본인 확인
          </button>
          <Link
  ...
  return (
    <div className="px-2 py-4 max-w-md mx-auto">
      ...
      <form
        ...
        className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6"
      >
        <div>
          <label className="block text-sm font-black text-penguin-black mb-2">
            투표 제목 수정 (필수)
          </label>
          <input
            required
        ...
        <p className="text-xs text-gray-400 font-bold bg-gray-50 p-4 rounded-xl leading-relaxed">
          💡 투표 선택지는 데이터 무결성을 위해 수정을 제한하고 있습니다.
          제목이나 설명만 수정 가능합니다.
        </p>
