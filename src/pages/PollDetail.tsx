...
const PollDetail: React.FC = () => {
  ...
  const fetchPollAndUserVote = async (pollId: string, currentVoterId: string) => {
    setLoading(true);
    try {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="mt-4 text-gray-500 font-bold">투표 데이터를 불러오는 중...</p>
      </div>
    );
  }
  ...
  if (!poll) {
    return (
      <div className="p-8 text-center font-bold text-penguin-black">
        투표를 찾을 수 없습니다.
        <br />
        <Link to="/" className="text-sm underline mt-4 inline-block">홈으로 돌아가기</Link>
      </div>
    );
  }

  const hasDescription = !!poll.description && poll.description.trim().length > 0;
      ...
      <div className="bg-white px-4 py-8 md:p-14 rounded-[2.5rem] md:rounded-[4rem] shadow-[0_20px_70px_rgb(0,0,0,0.08)] border border-gray-50 mb-8 relative overflow-hidden">
        <div className="flex flex-wrap justify-between items-center gap-2 mb-8 md:mb-10">
          <div className="flex items-center gap-2 md:gap-3">
             ...
             <span className="text-[10px] md:text-xs text-gray-400 font-black">{poll.total_votes}명 참여중</span>
          </div>
        ...
        {!myVote ? (
            ...
            {poll.options.map((option, index) => (
              ...
              >
                <span className={`text-[9px] font-black uppercase tracking-widest mb-2 block ${
                  index === 0 ? 'text-penguin-yellow/50' : 'text-penguin-black/40'
                }`}>
                  Option {String.fromCharCode(65 + index)}
                </span>
                <p className={`font-black text-base md:text-xl leading-snug relative z-10 ${
                  index === 0 ? 'text-penguin-yellow' : 'text-penguin-black'
                }`}>
                  {option.option_text}
                </p>
                <span className={`absolute -right-4 -bottom-4 text-7xl md:text-9xl opacity-10 font-black italic ${
          ...
          <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                return (
                  <div key={option.id} className="space-y-2 md:space-y-3">
                    <div className="flex justify-between items-end px-1 md:px-2 text-left gap-2 md:gap-4">
                      <span className={`text-base md:text-2xl font-black flex-1 ${isSelected ? 'text-penguin-black' : 'text-gray-400'}`}>
                        {option.option_text} {isSelected && '✅'}
                      </span>
                      <div className="flex items-baseline gap-1 md:gap-2 flex-shrink-0">
                        <span className="text-lg md:text-3xl font-black text-penguin-black tabular-nums">{option.vote_count}표</span>
                        <span className="text-xs md:text-lg font-bold text-gray-400 tabular-nums">({percent}%)</span>
                      </div>
                    </div>
                    <div className="w-full bg-gray-100 h-10 md:h-16 rounded-[1rem] md:rounded-[2rem] overflow-hidden flex shadow-inner border-[3px] md:border-4 border-gray-100">
                      <div className={`h-full transition-all duration-1000 relative min-w-0 ${
                        index === 0 ? 'bg-penguin-black' : 'bg-penguin-yellow'
                      }`} style={{ width: `${percent}%` }}>
                        <span className={`absolute left-4 md:left-6 top-1/2 -translate-y-1/2 font-black text-sm md:text-xl italic whitespace-nowrap ${
                          index === 0 ? 'text-penguin-yellow' : 'text-penguin-black'
                        }`}>
                          {String.fromCharCode(65 + index)}
                        </span>
            ...
            <div className="pt-6 text-center border-t border-gray-50">
               <p className="text-sm text-gray-400 font-black uppercase tracking-[0.1em]">총 {poll.total_votes}명이 참여했습니다!</p>
            </div>
          </div>
        )}
      </div>
