...
const CreatePoll: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('투표 생성 중 오류가 발생했습니다: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="px-2 py-4 max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-8 pb-12">
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border border-gray-100 space-y-6">
          ...
          <div>
            <label className="block text-sm font-black text-penguin-black mb-2 ml-1">상세 내용 (선택)</label>
            <textarea 
              value={description}
          ...
          <div className="space-y-4">
            <label className="block text-sm font-black text-penguin-black mb-1 ml-1">선택지 입력 (2개 고정)</label>
            <input 
              required
              type="text" 
              value={option1}
